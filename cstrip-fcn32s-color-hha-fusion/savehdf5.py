...
for counter, idx in enumerate(val_imgs):
    ...
    # colourArr = color_net.blobs['data'].data[0]
    ...
    score_colour_list.append(np.copy(color_net.blobs[layer].data))
    input_data_list.append(np.copy(color_net.blobs['data'].data))
    # if counter > 3:
...
for counter, idx in enumerate(val_imgs):
    hha_net.forward()
    print 'Should be loading  {} with index {}'.format(idx[0], idx[1])
    score_hha_list.append(np.copy(hha_net.blobs[layer + '_trip'].data))
    gt_hha_list.append(np.copy(hha_net.blobs[gt].data))
