...
parser.add_argument('--dataset_directory', default='/home/sean/hpc-home/Construction_Site/Springfield/12Aug16/K2')
...
parser.add_argument('--out_dir', default='/home/sean/Dropbox/Uni/Code/FCN_models/data/cs-trip')
...
for train_path in training_dir:
    ...
    img_fullnames = glob.glob(args.dataset_directory + '/'+ train_path+'/labels/*.mat')
...
for test_path in testing_dir:
    ...
    img_fullnames = glob.glob(args.dataset_directory + '/'+ test_path+'/labels/*.mat')
    for image_name in img_fullnames:
        img_index = getImgIndex(os.path.basename(image_name))
...
val_dir_index = train_dir_index[0:num_val_instances] + train_dir_index[-num_val_instances:]
